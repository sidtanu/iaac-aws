node {
   ...
   export VAULT_TOKEN="s.E5e8cyX1o6PkCk5PfaNDtu8Z"
   ...
   /opt/apps/terraform init
	...
	if [ "$infra" = "Y" ];then
		echo "yes" | /opt/apps/terraform plan -var "sg_name=aws-security-group-poc" -var "port=8080"
		echo "yes" | /opt/apps/terraform apply -var "sg_name=aws-security-group-poc" -var "port=8080"
	fi
	removeInfra=$(echo "$line" |awk -F "," '{ if ($5=="N") print $10; }')
	echo "$removeInfra"
	if [ "$removeInfra" = "Y" ]; then
		echo "yes" | /opt/apps/terraform destroy -var "sg_name=aws-security-group-poc" -var "port=8080"
	fi
    done
    } < ./../config.csv
